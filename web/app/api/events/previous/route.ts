...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = `events_previous_${limit}_${offset}`;
    ...
    const url = `${API_CONFIG.SPACE_DEVS.BASE_URL}/event/previous/?limit=${limit}&offset=${offset}`;
    ...
    return NextResponse.json(
      { error: 'Failed to fetch previous events' },
      { status: 500 }
    );
  }
}
